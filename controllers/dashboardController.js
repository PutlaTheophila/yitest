...
const Event = require('../models/eventModel');
const User = require('../models/userModel');
const Birthday = require('../models/birthdayModel'); // Assume file name
const asyncErrorHandler = require('../utils/asyncErrorHandler');
const { verifyToken } = require('../utils/jwt');
const Notification = require('../models/notificationModel.js');
const Coupon = require('../models/couponModel.js');
...
const getGreetings = asyncErrorHandler(async (req, res) => {
  const { userId } = req.params;
  const birthday = await Birthday.findOne({ birthdayUser: userId }).populate('greetings.greetedBy', 'name');
  if (!birthday) {
    return res.status(404).json({ status: 'fail', message: 'No birthday found' });
  }
  res.status(200).json({
    status: 'success',
...
const addGreeting = asyncErrorHandler(async (req, res) => {
  const token = req.headers.token;
  console.log('inside add greeting');
  console.log('hi',token);
  const { id: greetedBy } = await verifyToken(token);
  const { userId } = req.params;
  const { text } = req.body;
  let birthday = await Birthday.findOne({ birthdayUser: userId });
  if (!birthday) {
    birthday = await Birthday.create({ birthdayUser: userId });
  }
  birthday.greetings.push({ greetedBy, text });
  await birthday.save();
  const currentUser = await User.findById(greetedBy).select('name');
  const newGreeting = birthday.greetings[birthday.greetings.length - 1];
  newGreeting.greetedBy = { _id: greetedBy, name: currentUser.name };
  res.status(201).json({
    status: 'success',
...
module.exports = { sendData , test, getGreetings, addGreeting };